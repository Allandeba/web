export const POSITION = { up: 0, down: 1, left: 2, right: 3 }
const MIN_TOUCH_MOVEMENT = 20
...
function touchDirectionUp(position) {
  return position.y < touchStartY && touchStartY - position.y > MIN_TOUCH_MOVEMENT
}

function touchDirectionDown(position) {
  return position.y > touchStartY && position.y - touchStartY > MIN_TOUCH_MOVEMENT
}

function touchDirectionLeft(position) {
  return position.x < touchStartX && touchStartX - position.x > MIN_TOUCH_MOVEMENT
}

function touchDirectionRight(position) {
  return position.x > touchStartX && position.x - touchStartX > MIN_TOUCH_MOVEMENT
}

function updateSnakeDirection(position) {
  switch (position) {
    case POSITION.up: {
      if (canSnakeMove(POSITION.up)) snakeDirection = { x: 0, y: -INCREMENT_SIZE }
      break
    }

    case POSITION.down: {
      if (canSnakeMove(POSITION.down)) snakeDirection = { x: 0, y: INCREMENT_SIZE }
      break
    }

    case POSITION.left: {
      if (canSnakeMove(POSITION.left)) snakeDirection = { x: -INCREMENT_SIZE, y: 0 }
      break
    }

    case POSITION.right: {
      if (canSnakeMove(POSITION.right)) snakeDirection = { x: INCREMENT_SIZE, y: 0 }
      break
    }
  }
}
